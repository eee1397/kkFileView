...
public class KjshFileController {

    private final Logger logger = LoggerFactory.getLogger(KjshFileController.class);

    private final String fileDir = ConfigConstants.getFileDir();
    private final static String KJSH_ROOT_DIRNAME = "kjsh";

    private final static String KJSH_ROOT_PATH = KJSH_ROOT_DIRNAME + File.separator;
    private final static String KJSH_DEFAULT_PATH = "kjsh/coalId/report";
    public static final String BASE64_DECODE_ERROR_MSG = "Base64解码失败，请检查你的 %s 是否采用 Base64 + urlEncode 双重编码了！";


    /**
     * 存储路径： kjsh/coalId2/report/xxx/aaa.bbb
     *
     * @param coalId 2
     * @param bizDir /report/xxx/
     * @param file
     * @return
     */
    @PostMapping("/fileUpload")
    public ReturnResponse<Object> fileUpload(
            @RequestParam(value = "coalId", defaultValue = "coalId") String coalId,
            @RequestParam(value = "bizDir", defaultValue = KJSH_DEFAULT_PATH) String bizDir,
                                             @RequestParam("file") MultipartFile file) {

        String parentDir = KJSH_ROOT_PATH + coalId + bizDir;
        ReturnResponse<Object> checkResult = this.fileUploadCheck(file);
        if (checkResult.isFailure()) {
            return checkResult;
        }
        File outFile = new File(fileDir + parentDir);
        if (!outFile.exists() && !outFile.mkdirs()) {
            logger.error("创建文件夹【{}】失败，请检查目录权限！", fileDir + parentDir);
        }
        String fileName = checkResult.getContent().toString();
        logger.info("上传文件：{}{}{}", fileDir, parentDir, fileName);
        try (InputStream in = file.getInputStream(); OutputStream out = Files.newOutputStream(Paths.get(fileDir + parentDir + fileName))) {
    ...
    public ReturnResponse<Object> deleteFile(HttpServletRequest request, String fileName, String password) {
        ...
        File file = new File(fileDir + KJSH_ROOT_PATH + fileName);
    ...
    public List<Map<String, String>> getFiles() {
        ...
        File file = new File(fileDir + KJSH_ROOT_PATH);
        if (file.exists()) {
            File[] files = Objects.requireNonNull(file.listFiles());
            Arrays.sort(files, (f1, f2) -> Long.compare(f2.lastModified(), f1.lastModified()));
            Arrays.stream(files).forEach(file1 -> {
                Map<String, String> fileName = new HashMap<>();
                fileName.put("fileName", KJSH_ROOT_DIRNAME + "/" + file1.getName());
                list.add(fileName);
            });
        }
        return list;
